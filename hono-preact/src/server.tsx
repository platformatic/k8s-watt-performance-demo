...
// ── Layout (with hydration data like other frameworks) ──
    ...
    footer { margin-top: 64px; padding: 32px 0; border-top: 1px solid #e5e7eb; text-align: center; color: #9ca3af; font-size: 14px; }
      ...
      <body>
        ...
        <script type="module" dangerouslySetInnerHTML={{ __html: `
          // Client hydration stub — picks up SSR data for interactive components
    ...
    <Layout title="Home" ssrData={{ games, trending, releases }}>
      ...
      <section style="margin-bottom:48px">
        ...
        <div style="display:grid;grid-template-columns:repeat(5,1fr);gap:16px">
          {games.map((game: any) => (
            <a href={`/games/${game.slug}`} class="card" style="text-decoration:none;color:inherit;padding:16px" data-interactive>
      ...
      <section style="margin-bottom:48px">
        ...
        <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:16px">
          {trending.map((card: any) => (
            <a href={`/cards/${card.id}`} class="card" style="text-decoration:none;color:inherit;padding:16px" data-interactive>
      ...
      <section>
        ...
        <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:16px">
          {releases.map((set: any) => (
            <a href={`/sets/${set.slug}`} class="card" style="text-decoration:none;color:inherit;padding:16px" data-interactive>
  ...
  return (
    <Layout title="Games" ssrData={{ games }}>
      ...
      <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:24px">
        {games.map((game: any) => (
          <a href={`/games/${game.slug}`} class="card" style="text-decoration:none;color:inherit;padding:24px" data-interactive>
        ...
        <section>
          ...
          <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:16px">
            {game.sets.map((set: any) => (
              <a href={`/sets/${set.slug}`} class="card" style="text-decoration:none;color:inherit;padding:20px" data-interactive>
...
function SetDetailPage({ set, page, slug }: { set: any; page: number; slug: string }) {
  return (
    <Layout title={set.name} ssrData={{ set }}>
      <div style="margin-bottom:32px">
      ...
      <div style="display:grid;grid-template-columns:repeat(6,1fr);gap:16px">
        {set.cards.map((card: any) => (
          <a href={`/cards/${card.id}`} class="card" style="text-decoration:none;color:inherit;padding:12px" data-interactive>
...
function CardDetailPage({ card, sellers, sets, games }: { card: any; sellers: any[]; sets: any[]; games: any[] }) {
...
function SearchPage({ results, games, q, game, page }: { results: any; games: any[]; q?: string; game?: string; page: number }) {
  return (
    <Layout title="Search" ssrData={{ results, games }}>
    ...
    <Layout title="Sellers" ssrData={{ sellers }}>
      ...
      <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:24px">
        {sellers.map((seller: any) => (
          <a href={`/sellers/${seller.slug}`} class="card" style="text-decoration:none;color:inherit;padding:24px" data-interactive>
...
function SellerDetailPage({ data, page, slug, cards }: { data: any; page: number; slug: string; cards: Map<string, any> }) {
  return (
    <Layout title={data.name} ssrData={{ seller: data }}>
      <div style="margin-bottom:32px">
        ...
        <table style="width:100%;border-collapse:collapse">
          ...
          <tbody>
            {data.listings.map((listing: any) => {
              const card = cards.get(listing.cardId) as any;
              return (
                <tr style="border-top:1px solid #e5e7eb">
                  <td style="padding:12px 16px">
  ...
  return (
    <Layout title="Cart" ssrData={{ items: [] }}>
      ...
      <div class="card" style="padding:32px;text-align:center">
        ...
        <a href="/search" style="display:inline-block;background:#2563eb;color:white;padding:12px 24px;border-radius:8px;text-decoration:none" data-interactive>Browse Cards</a>
      </div>
    </Layout>
  );
}

// ── Hono routes with Preact SSR ──
...
app.get('/', async (c) => {
  const [games, trending, releases] = await Promise.all([
    db.getGames(), db.getTrendingCards(8), db.getNewReleaseSets(4),
  ]);
...
app.get('/games/:slug', async (c) => {
  const game = await db.getGameWithSets(c.req.param('slug'));
...
app.get('/sets/:slug', async (c) => {
  ...
  return render(<SetDetailPage set={set} page={page} slug={slug} />);
});
...
app.get('/cards/:id', async (c) => {
  ...
  const [sellers, sets, games] = await Promise.all([
    db.getSellers(), db.getSets(), db.getGames(),
  ]);
  return render(<CardDetailPage card={card} sellers={sellers} sets={sets} games={games} />);
});
...
app.get('/search', async (c) => {
  ...
  return render(<SearchPage results={results} games={games} q={q} game={game} page={page} />);
});
...
app.get('/sellers/:slug', async (c) => {
  ...
  const cardList = await db.findMany<any>('cards', data.listings.map((l: any) => l.cardId));
  const cardMap = new Map(cardList.map((card: any) => [card.id, card]));
  return render(<SellerDetailPage data={data} page={page} slug={slug} cards={cardMap} />);
});
...
db.initialize().then(() => {
  serve({ fetch: app.fetch, port }, () => {
    console.log(`Hono+Preact SSR server running on http://localhost:${port}`);
  });
});
